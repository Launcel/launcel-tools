...
import xyz.x.common.utils.StringUtils;
...
public class UploadProperties
{
    ...
    {
        this.domain = StringUtils.isBlank(domain) ? "" : domain;
        UploadConfig.domain = this.domain;
    }

    public void setMaxSize(Long maxSize)
    {
        this.maxSize = Objects.isNull(maxSize) ? Long.MAX_VALUE : maxSize;
        UploadConfig.maxSize = this.maxSize;
    }

    public void setMinSize(Long minSize)
    {
        this.minSize = Objects.isNull(minSize) ? 1024L : minSize;
        UploadConfig.minSize = this.minSize;
    }

    public void setPath(String path)
    {
        this.path = StringUtils.isNotBlank(path) ? path : File.separator.concat("Users")
                .concat(File.separator)
                .concat("tmp")
                .concat(File.separator)
                .concat("upload");
        UploadConfig.path = this.path;
    }

    public void setFileType(List<String> fileType)
    {
        this.fileType = CollectionUtils.isEmpty(fileType) ? Arrays.asList("doc", "docx", "pdf", "jpg", "png", "jepg") : fileType;
        UploadConfig.fileType = this.fileType;
    }

    public void setContentType(List<String> contentType)
    {
        this.contentType = CollectionUtils.isEmpty(contentType) ? Arrays.asList("504b0304", "d0cf11e0", "25504446", "ffd8ffe0", "ffd8ffe1", "89504e47",
                "ffd8ff") : contentType;
        UploadConfig.contentType = this.contentType;
    }
